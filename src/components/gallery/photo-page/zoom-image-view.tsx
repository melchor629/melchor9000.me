...
const calculateTranslation = (imageSize: { width: number; height: number; }, normCoords: Coords): Coords => {
    const margin = 100;
...
const translate = (coords: Coords, scale?: number) =>
    `translate3d(${coords[0]}px, ${coords[1]}px, 0px) scale(${scale || 1})`;
const clamp = (v: number, min: number, max: number) => Math.max(min, Math.min(v, max));

const calculateInitialTranslation = (imageSize: { width: number; height: number; }, imageView: HTMLDivElement) => {
...
const ZoomImageViewImpl = ({ currentSizeImageUrl, onClose, initialMousePosition, photo, imageViewRef }: ZoomImageOverlayProps) => {
    ...
    return (
        <animated.div className="zoom-modal"
            style={ modalStyles }
            ...
            <animated.div className="zoom-container" style={ containerStyles }>
                <img src={ currentSizeImageUrl }
                    className="img-small"
                    alt={ photo.info!.description._content || 'Zoomed image' } />
                <img src={ bestQualityImage.url }
                    className="img-big"
                    alt={ photo.info!.description._content || 'Zoomed image' } />
            </animated.div>
        </animated.div>
