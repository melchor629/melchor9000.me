...
import type { VizStateToProps } from '../containers/viz'
...
export default class Viz extends React.Component<VizProps, VizState> {
  ...
  private play(startPos?: number) {
    const { audioSource: currentAudioSource } = this.state
    const { t } = this.props
    ...
    this.player.playSound('song', startPos).then((audioSource) => {
      ...
      .catch((error) => Viz.showError(t('viz.cannotPlay') + error.toString()))
  }
  ...
  private seekBarEnd() {
    const { timePosition } = this.state
    this.play(timePosition!)
