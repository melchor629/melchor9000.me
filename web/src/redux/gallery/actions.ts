...
import type { GalleryPhotoSizes, GalleryState } from './reducers'
...
export const loadMorePhotos = (
  ...
  const { galleryList: { totalPhotos, photos: loadedPhotos } } = getState()
...
const loadDetailedPhotoImpl = debounce(async (
  ...
  dispatch(loadedPhotoDetail({
    photoId: photo.id, exif, info, sizes,
  }))
}, 250)
